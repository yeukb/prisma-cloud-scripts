...
curl -s -X GET \
    ...
    TZ=$timezone jq '[ .[] | select(.result=="Failed")  |{date: (.timestamp / 1000 | strflocaltime("%a %Y-%m-%d %H:%M:%S %Z")), user: .user, ipAddress: .ipAddress, resourceType: .resourceType, resourceName: .resourceName, action: .action, result: .result} ]'
